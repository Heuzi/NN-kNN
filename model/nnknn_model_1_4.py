...
def train_model(X_train, y_train, X_val, y_val, feature_extractor, cfg): # , glocal_fw_set_num=glocal_fw_set_num
    ...
    for epoch in range(training_epochs):
        # ---- Unfreeze feature extractorat scheduled epoch ----
        if freeze_epochs > 0 and model.feature_extractor is not None and epoch == freeze_epochs:
            for p in model.feature_extractor.parameters():
                p.requires_grad_(True)
            print(f"[unfreeze] feature_extractor unfrozen at epoch {epoch}")
        # ---- case-normalizer warmup schedule ----
        ...
        # ===== Adaptation warm-up: retrieval only, then turn adaptation on =====
        adapt_warm = int(cfg.get("adapt_warmup_epochs", 0))
        adapt_ramp = int(cfg.get("adapt_lr_ramp_epochs", 0))  # lr ramp only (no output blending)

        adapt_on = (epoch >= adapt_warm)
        model.adapt_enabled = adapt_on

        # Also freeze adapter updates during warmup by setting its LR to 0
        if len(optimizer.param_groups) > 2 and len(optimizer.param_groups[2]["params"]) > 0:
            if not adapt_on:
                optimizer.param_groups[2]["lr"] = 0.0
            else:
                if adapt_ramp > 0:
                    t = min(1.0, (epoch - adapt_warm) / max(1, adapt_ramp))
                    optimizer.param_groups[2]["lr"] = base_adapter_lr * t
                else:
        ...
        for X_batch, y_batch in train_loader:
            ...
            if task == "classification":
                ...
                if not base.requires_grad:
                    # hard_knn: non-differentiable, nothing to optimize
                    continue
                ...
                if cfg.get("regression_locality", False):
                    ...
                    comps["loc_scale"] = torch.tensor(loc_scale, device=base.device)
                else:
                    loss, comps = base, {}
            ...
            if lambda_case_bias != 0.0 and hasattr(model, "biases") and model.biases.requires_grad:
                ...
                loss = loss + lambda_case_bias * bias_l2
                # keep logging consistent
                if isinstance(comps, dict):
                    comps["case_bias_l2"] = bias_l2.detach()
            loss.backward()
...
def reg_locality_reg_loss(
    ...
    lambda_exp   = cfg.get("lambda_expdist", 0.0)
